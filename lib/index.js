...
module.exports = options => (files, metalsmith, done) => {
  ...
  // Let the user know when there are no files to process, unless the check is
  // suppressed
  if (validFiles.length === 0) {
    if (settings.suppressNoFilesError === true) {
      debug('No files to process, ignoring because "suppressNoFilesError" is true');
    } else {
      done(
        new Error(
