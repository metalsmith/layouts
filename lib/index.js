 ...
 *   @property {String|RegExp|Object} pattern (optional)
...
function plugin(opts){
  ...
  // Accept string for pattern
  if (typeof opts.pattern === 'string' || opts.pattern instanceof RegExp) {
    opts.pattern = { path: opts.pattern };
  }

  // Pattern strings should be converted to RegExp (except for path)
  if (opts.pattern) {
    Object.keys(opts.pattern).forEach(function (key) {
      if (typeof opts.pattern[key] === 'string' && key != 'path') {
        opts.pattern[key] = new RegExp(opts.pattern[key]);
      }
    });
  }
