...
function plugin(opts){
  ...
  var def = opts.default;
	//- a folder to look for template files
	//- must be relative to metalsmith.directory()
  var dir = opts.directory || 'layouts';
	//- the name of the engine consolidate should use
	//- engine = consolidate[opts.engine]
  var engine = opts.engine;
	//- ignore any file found in the partials folder
	//  that does not have this extension
	//- ignore none, if this setting is missing
  ...
  var partials = opts.partials;
	//- ignore files if their path won't match this pattern
  ...
  return function(files, metalsmith, done){
    ...
    var templates = {};
		...
		//- an absolute path to the layout folder
		...
		//- with path values relative to the layouts folder
		var partialsDir = {};
		
		//- partialsMap = { key: value }
		//- key = absolute path to a layout file
		//- value = transformed partials container
		//  with path values transformed to be relative to the layout file
		var partialsMap = {};
    ...
    Object.keys(files).forEach(function(file){
      ...
      data.contents = data.contents.toString();
			...
			//- an absolute path to the file's template file to use
			//  [metalsmith.directory() / opts.dir / template-file]
			...
			if(partials) {
				...
				if( !partialsRel) {//- no entry found
					...
					if(templateDir === layoutsAbs) {
						...
						//- template file is located inside a subfolder
						//- consolidate expects partial path values to be
						//  relative to the template file - issue #126
						//- transform these values to solve this issue
				...
				//- cant tell if extend() is necessary ???
				clone.partials = extend({}, partialsRel);
			}

			//from consolidate.js
			//- exports.handlebars = fromStringRenderer('handlebars')
			//- fronStringRenderer will return a function
			//  void function(path, options, callback)
			//  path = path to the template file
			//- this function will first read the partials like so:
    ...
    each(templates, convert, renderFiles);
		...
		// Is a cleanup necessary ???
		//- partialsContainer, map
		return;
  };
}
